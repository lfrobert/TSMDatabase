...
select FIRST_NAME || ' ' || LAST_NAME as "Full Name", EMAIL, CELL_PHONE, 'Null' as " Extra Contact Info"  from TSM_EMPLOYEE union all
select FIRST_NAME || ' ' || LAST_NAME as "Full Name", EMAIL, CELL_PHONE, HOME_PHONE from TSM_CUSTOMER
with read only;
...
select VEHICLE_TYPE, VEHICLE_YEAR, VEHICLE_MAKE || ' ' || VEHICLE_MODEL as "MAKE / MODEL", COLOR, OPERATIONAL_YN from TSM_VEHICLE ORDER BY VEHICLE_TYPE
with read only;



--3
CREATE VIEW CommunityLocations AS
SELECT DISTINCT(STREET), CITY, STATE, ZIP_CODE FROM TSM_RESIDENT
with read only;



--4
CREATE VIEW EMPLOYEETYPE AS
select EMPLOYEE_ID, FIRST_NAME || ' ' || LAST_NAME as "Full Name", MOVER_YN, office_worker_yn from TSM_EMPLOYEE
with read only;
